...
#include <atomic>  // FPH: for the high-water register - see take_high_water()
#include <memory>
...
class WakeWordModel final : public StreamingModel {
 public:
  ...
  // FPH: the high-water register (FutureProofHomes vendored addition - see FPH_VENDOR.md; upstream
  // FPH: PR intended). The max sliding-window mean since the last take, kept by determine_detected()
  // FPH: on the inference task and drained by mww_runtime_loader from the main loop. It is the one
  // FPH: number that makes sub-cutoff scores observable - room-pressure telemetry, scored close
  // FPH: calls, and the tuner's max-mean all ride it. Single writer / single reader; the benign race
  // FPH: between the writer's read-max-store and the taker's exchange can lose at most one sample.
  uint8_t take_high_water() { return this->fph_high_water_.exchange(0, std::memory_order_relaxed); }  // FPH
 ...
 protected:
  ...
  // FPH: see take_high_water() above.
  std::atomic<uint8_t> fph_high_water_{0};  // FPH
