...
DetectionEvent WakeWordModel::determine_detected() {
  ...
  // FPH: high-water register - keep the max window mean since the last take_high_water() drain.
  if (detection_event.average_probability > this->fph_high_water_.load(std::memory_order_relaxed))  // FPH
    this->fph_high_water_.store(detection_event.average_probability, std::memory_order_relaxed);    // FPH
