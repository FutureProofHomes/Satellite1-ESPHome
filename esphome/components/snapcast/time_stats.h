...
public:
    ...
    if (!has_value_) {
      ...
      if (pending_init_values_.size() >= min_valid_samples_) {
        auto best = std::min_element(
            pending_init_values_.begin(), pending_init_values_.end(),
        ...
        if (hybrid_) reset_kalman_(received_time, ema_);
      }
      return;
    }
  ...
  void set_hybrid(bool hybrid_kalman) {
    if (hybrid_ == hybrid_kalman) return;
...
public:
  ...
  int64_t current_sigma_us_() const {
    ...
    if (!rtt_inited_) return static_cast<int64_t>(std::llround(floor_sigma));
    ...
    if (z < 0.0) z = 0.0;
    if (z > cfg_.sigma_z_cap) z = cfg_.sigma_z_cap;
  ...
  void adaptive_kalman_update_(tv_t t_now, tv_t meas, tv_t rtt) {
    ...
    if (outlier) {
      ...
      if (z_rtt < 0.0) z_rtt = 0.0;
      if (z_rtt > cfg_.sigma_z_cap) z_rtt = cfg_.sigma_z_cap;

      const double inflate = cfg_.inflate_base + cfg_.inflate_slope * z_rtt;
      P00 *= inflate; P01 *= inflate; P10 *= inflate; P11 *= inflate;
    }
    ...
    last_kalman_offset_tv_ = tv_t::from_microseconds(
        static_cast<int64_t>(std::llround(x_off_us_)));
  }
};
